...
/**
 * Live-book event streaming endpoint (SSE).
 *
 * Uses Server-Sent Events (SSE) instead of WebSocket for one-way server-to-client
 * streaming of job progress events. The URL path `/api/live-book/ws` is kept for
 * backward compatibility with the original API contract.
 *
 * Query params:
 *   - book_id: Required. The live-book ID to subscribe to events for.
 */
